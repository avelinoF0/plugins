...
KEYS = {
            ...
            '1m':{'node' : 'os:cpu:load_average', 'key' :'avg_1m'}, # Load Avg 1m
            '5m':{'node' : 'os:cpu:load_average', 'key' :'avg_5m'}, # Load Avg 1m
            '15m':{'node' : 'os:cpu:load_average', 'key' :'avg_15m'}, # Load Avg 1m
            ...
            'total_in_bytes' : {'node' :'fs:total','key':'fs_total_in_bytes'},
            'free_in_bytes' : {'node' :'fs:total','key':'fs_free_in_bytes'},
            'available_in_bytes' : {'node' :'fs:total','key':'fs_available_in_bytes'},
}
...
class Elasticsearch():
    def __init__(self,host_name,port,node_name,username,password):
        self.data = {}
        self._url = "https://"+host_name+':'+port+"/_nodes/stats"
...
if __name__ == '__main__':
    ...
    if 'msg' not in result:
        disk_total = result['fs_total_in_bytes']
        disk_used = result['fs_total_in_bytes'] - result['fs_free_in_bytes']
        disk_percent = int(disk_used / disk_total * 100)
        result['fs_disk_usage_percent'] = disk_percent
